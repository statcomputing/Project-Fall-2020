...
Based on the above properties firstly, we calculated the ratio of symmetric quantiles, where $I$ lies between 0 and 1, defined by:
...
\end{aligned}
$$
Then, calculate the confidence intervals for $R(p)$ of the form 
$\hat{R}(p)\pm z_{1-\alpha}\hat{\sigma}_{p}/\sqrt{n}$, where $\hat{\sigma}_{p}$ requires estimates of the quantile density at $p/2$ and $1-p/2$ and confidence intervals for $I$ are estimated by $\hat{I} = 1 - \int_{0}^{1}\hat{R}(p)dp$. For each $0<p<1$, $\hat{R}(p)$ is defined as $\frac{\hat{x}_{p/2}}{\hat{x}_{1-p/2}}$. 
...
```{r, echo=FALSE, fig.cap="Income inequality curve and Lorenz curve of Disposable Personal Income in the US between 1959 and 2020", fig.align="center",out.width = '40%'}
...
```{r, echo=FALSE, fig.cap="Income inequality curve and Lorenz curve of Earnings Data of Women in the US between 1979 and 2020", fig.align="center",out.width = '40%'}
knitr::opts_chunk$set(fig.pos = 'H')
...
https://www.tandfonline.com/doi/abs/10.1080/00031305.2017.1366366#:~:text=Ratios%20of%20quantiles%20are%20often,ratios%20has%20recently%20become%20available.&text=The%20area%20above%20the%20curve,easily%20interpretable%20measure%20of%20inequality.
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2652960/
http://www.bu.edu/eci/files/2019/10/Principles_2e_Ch11.pdf
https://www.nature.com/articles/s41599-020-0484-6
